  ...
  if [[ ! -z "$ALL_BACKUP" ]]; then
    for FILE in $ALL_BACKUP; do
      ...
      if [ -e $DIR_TO_NFS_WEEKLY/$(basename "$FILE") ]; then
        ...
        echo "Some error, backup in $DIR_TO_NFS_WEEKLY not exists."
      fi
    done
  else
    echo "$DIR_TO_BACKUP is empty, Backup not create"
  fi
}
...
rotate_backups() {
  ...
  if [[ -e "$DIR_TO_NFS_MONTHLY/$LAST_BACKUP" ]]; then
    ...
    echo "Monthly Backup in $DIR_TO_NFS_MONTHLY not exists"
  fi
